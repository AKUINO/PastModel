...
# PARAMÈTRES INITIAUX
# ======================
raccords = 20 # raccords rétrécisants de 9,5mm à 8,7mm
...
if pasto == "2022":
    ...
    fluid = FLUID_WATER
    debit = 0.145/ 3600  # m³/s (300 L/h)
    temp_froid_in = 13.5         # °C (lait entrant)
    temp_chaud_in = 85        # °C (lait sortant du pasteurisateur)
    ...
    longueur_plaque = 0.38  # m, longueur d'une plaque
    largeur_plaque = 0.10
    surface_plaque = longueur_plaque * largeur_plaque #0.043  # m², surface d'une plaque
    espacement = 0.0021 #volume_plaque / surface_plaque
    # Calcul dérivés
    ...
    epaisseur_plaque = 0.001 #m = 1.5mm d'épaisseur de plaque
...
obstacles = [
    ('coudes', 0.4, diam_jonction, 6),      # 6 coudes
    ('retrecissement', diam_jonction, diam_jonction-0.0007, 18),  # 18 rétrécissements 9.5→8.7mm
    ('elargissement', diam_jonction-0.0007, diam_jonction, 18)    # 18 élargissements 8.7→9.5mm
]
...
def proprietes_fluide(fluid: str, temp_C: float) -> dict:
    ...
    # Viscosité dynamique du lait (μ) ≈ 2,0 × 10⁻³ Pa·s (pour du lait à 4% de matière grasse à 20°C)
        ...
        mu = 226.9503858568 * np.exp(-0.0162557770797138 * temp_K) / 1000
        ...
        rho = 1048.2 - 0.488*temp_C + 0.0012*temp_C**2
        # Viscosité dynamique (Pa.s)
        mu = 4.85e-7 * np.exp(3279.4 / temp_K)# 20°C=15-115 mPas
        # Chaleur spécifique (J/kg.K)
        cp = 3180 + 8.5*temp_C
        # Conductivité thermique (W/m.K)
...
m_eau = debit * eau_target['rho']  # kg/s
cp_eau = eau_target['cp']

v_min = 1.5 # m/s vitesse minimale dans le pasteurisateur
temp_froid_out_target = temp_chaud_in - (appoint / (cp_eau * m_eau))
...
m_eau = debit * eau_mitigee['rho']  # kg/s
cp_eau = eau_mitigee['cp']
q_transfert = m_eau * cp_eau * (temp_froid_out_target - temp_froid_in)
...
def coefficient_convection_serpentin(fluid: str, parallele: int,
    ...
    if section_type == 'tube':
        ...
        A = np.pi * ((diam_calandre/2)**2 - (diam_ext_tube/2)**2)
    ...
    if Re > 2000:
        ...
        Nu = (f/8) * (Re - 1000) * props['pr'] / (1 + 12.7 * np.sqrt(f/8) * (props['pr']**(2/3) - 1))
    ...
    if section_type == 'tube':
        f_turbulent = (0.3164*(Re**-0.25))+(0.03*(De**-0.3))
    else:
...
def coefficient_convection_plaque(fluid: str, section_canal: float, longueur: float, angle_chevron: int, debit: float,
                                  temp: float, parallele: float) -> tuple:
    ...
    f_canal = 2 * f * (longueur/d_h) * (props['rho'] * v**2 / 2)
    f_entree_sortie = 1.4 * (props['rho'] * v**2 / 2)
    delta_P = f_canal + f_entree_sortie
...
if a_plaque is None:
    ...
    h_tube, f_tube, rey_tube, v_tube, delta_P_tube = \
        coefficient_convection_plaque(fluid, section_canal, longueur, angle_chevron, debit, temp_tube, parallele)
...
if a_plaque is None:
    ...
    print(f"- Dimension: {longueur_plaque:.3f}m de longueur x {largeur_plaque:.3f}m de largeur, surface={surface_plaque:.6f}m²")
    print(f"  {espacement*mmInM:.1f}mm d'espacement entre les plaques")
print("\nTempératures:")
...
print(f"\nPertes de charge:")
if a_plaque is None:
    print(f"- Côté froid (annulaire), diamètre {diam_calandre*mmInM:.1f} x {diam_ext_tube*1000:.1f}mm, long={longueur:.1f}m: {doc_P(delta_P_annulaire, P2_annulaire)}")
else:
    print(f"- Côté froid: {doc_P(delta_P_annulaire, P2_annulaire)}")
...
print(f"- Chauffe, diamètre={diam_chauffe*mmInM:.1f}mm, long={longueur_chauffe:.1f}m: {doc_P(delta_P_chauffe, P2_chauffe)}")

print(f"- Maintien {duree_maintien:.1f}s, diamètre={diam_maintien*mmInM:.1f}mm, long={longueur_maintien:.1f}m: {doc_P(delta_P_maintien,P2_maintien)}")
if a_plaque is None:
    print(f"- Côté chaud, diamètre {diam_int_tube*mmInM:.1f}mm, long={longueur:.1f}m: {doc_P(delta_P_tube,P2_tube)}")
else:
    print(f"- Côté chaud: {doc_P(delta_P_tube,P2_tube)}")
print(f"- Jonctions diamètre={diam_jonction*mmInM:.1f}mm, long={long_jonction:.1f}m: {doc_P(delta_P_jonction,P2_jonction)}")

print(f"- TOTAL, long= {longueur*2+longueur_chauffe+longueur_maintien+long_jonction:.3f}m:",
...
if a_plaque is None:
    ...
    h_tube, f_tube, rey_tube, v_tube, delta_P_tube = \
        coefficient_convection_plaque(cleaning_fluid, section_canal, longueur, angle_chevron,
                                      debit_nettoyage, temp_nettoyage, parallele)
